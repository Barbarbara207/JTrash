...
public class PlayerHandModel {
    ...
    /**
     * Costruttore della classe PlayerHandModel
     */
    public PlayerHandModel(ArrayList<CardModel> carte, int quanteVittorie) {
        this.carte = carte;
     ...
     * della mano del giocatore, permettendo alla View di rappresentarla correttamente
     */
    public String[] listaManoGiocatore() {
        ArrayList<String> stringManoGiocatore = new ArrayList<>();
        for (CardModel carta : carte) {
            stringManoGiocatore.add(carta.getValore() + " of " + carta.getSegno() + " of " +carta.isScoperta());
        }
        stringManoGiocatore.add(0, "ManoGiocatore");

        return stringManoGiocatore.stream().toArray(String[] :: new);
    }
